...
ddir = 'sims_gauss_fullsky_ns256_csd_std0.3_gm3/'
sdir = './data/sim03/'
...
def clean_maps(k, fn):
    testmap = hp.read_map(fn+'maps_sky_signal.fits', field=np.arange(12), verbose=False)
    Qs = testmap[::2]
    Us = testmap[1::2]
    ...
    print("Saving")
    np.savez(sdir+'hybrid_params'+str(k), params=rdict['params_ML'], Sbar=Sbar, Q=Q)
    hp.write_map(sdir+'residualmaps'+str(k)+'.fits', reducedmaps.reshape((12, hp.nside2npix(256))), overwrite=True)
    return
